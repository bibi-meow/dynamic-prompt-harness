...
from dynamic_prompt_harness.core.io_contract import Entry, AbstractInput, Decision
from dynamic_prompt_harness.core.logger import JsonlLogger

def _entry(cmd, timeout=5.0):
    return Entry(id="t", triggers=("pre_tool_use",), command=tuple(cmd),
                 priority=0, timeout_sec=timeout, log_level=None, matcher=None)
...
def test_timeout_is_deny(tmp_path):
    ex = Executor(tmp_path, JsonlLogger(tmp_path/"l.log"))
    r = ex.execute(_entry([sys.executable, "-c", "import time; time.sleep(5)"], timeout=0.3), _input())
    assert r.decision is Decision.DENY
...
def test_entry_env_vars_exposed(tmp_path):
    code = ('import os,json,sys;'
            'json.dump({"decision":"hint","message":os.environ["DPH_ENTRY_ID"]+"/"+os.environ["DPH_TRIGGER"]}, sys.stdout)')
    ex = Executor(tmp_path, JsonlLogger(tmp_path/"l.log"))
    r = ex.execute(_entry([sys.executable, "-c", code]), _input())
