...
from .io_contract import Entry, AbstractInput, AbstractResult, Decision
...
class Executor:
    ...
    def execute(self, entry: Entry, input: AbstractInput) -> AbstractResult:
        stdin_payload = json.dumps({
            "trigger": input.trigger, "tool": input.tool,
            "tool_input": input.tool_input, "tool_result": input.tool_result,
            "prompt": input.prompt, "context": input.context,
        })
        env = {**os.environ, "DPH_ENTRY_ID": entry.id, "DPH_TRIGGER": input.trigger}
        if entry.log_level:
            env["DPH_LOG_LEVEL"] = entry.log_level
        try:
            proc = subprocess.run(
                list(entry.command), input=stdin_payload, capture_output=True,
                text=True, timeout=entry.timeout_sec, cwd=str(self._cwd), check=False,
                env=env,
            )
            ...
            return AbstractResult(Decision.DENY, f"[{entry.id}] oserror: {e}", {"reason": "oserror"})

        if proc.returncode != 0:
            self._log.log("error", "executor_nonzero", entry_id=entry.id,
                          rc=proc.returncode, stderr=proc.stderr[:500])
            return AbstractResult(Decision.DENY, f"[{entry.id}] exit {proc.returncode}: {proc.stderr.strip()}",
        ...
        try:
            ...
            return AbstractResult(Decision.DENY, f"[{entry.id}] invalid decision JSON",
